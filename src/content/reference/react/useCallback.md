...
`useCallback` एक React हुक है जो आपको रेंडर के बीच फंक्शन डेफिनिशन को कैश करने की सुविधा देता है।
...
अपने कम्पोनेंट के टॉप लेवल पर फंक्शन डेफिनिशन को रेंडर के बीच कैश करने के लिए `useCallback` कॉल करें:
...
* `dependencies`: उन सभी रिएक्टिव वैल्यूज़ की सूची जो `fn` के कोड में रेफरेंस की गई हैं। रिएक्टिव वैल्यूज़ में प्रॉप्स, स्टेट, और कम्पोनेंट बॉडी में डायरेक्टली डिक्लेयर किए गए वेरिएबल्स और फंक्शन्स शामिल हैं। यदि आपका लिंटर [React के लिए कॉन्फ़िगर](/learn/editor-setup#linting) है, तो यह वेरिफ़ाई करेगा कि हर रिएक्टिव वैल्यू सही तरीके से डिपेंडेंसी के रूप में निर्दिष्ट है। डिपेंडेंसीज़ की सूची की संख्या स्थिर होनी चाहिए और इसे इनलाइन, `[dep1, dep2, dep3]` की तरह लिखा जाना चाहिए। React प्रत्येक डिपेंडेंसी की तुलना पिछले रेंडर से [`Object.is`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/ F/Global_Objects/Object/is) एल्गोरिदम का उपयोग करके करता है।
...
* `useCallback` एक हुक है, इसलिए इसे केवल आपके कम्पोनेंट या कस्टम हुक के **टॉप लेवल** पर ही कॉल किया जा सकता है। इसे लूप्स या कंडीशन्स के अंदर कॉल नहीं करना चाहिए। यदि ज़रूरी हो, तो नया कम्पोनेंट बनाएँ और स्टेट को वहाँ ले जाएँ।

* React कैश्ड फंक्शन को तब तक डिस्कार्ड **नहीं करता जब तक कोई विशेष कारण न हो**। उदाहरण के लिए, डेवलपमेंट मोड में, यदि आप कम्पोनेंट की फ़ाइल एडिट करते हैं, तो React कैश डिस्कार्ड कर देता है। डेवलपमेंट और प्रोडक्शन दोनों में, यदि कम्पोनेंट इनिशियल माउंट के दौरान सस्पेंड होता है, तो React कैश डिस्कार्ड कर देता है। भविष्य में, React और फ़ीचर्स जोड़ सकता है जो कैश डिस्कार्ड का लाभ उठाएँ—जैसे कि वर्चुअलाइज़्ड लिस्ट्स के लिए बिल्ट-इन सपोर्ट, जो वर्चुअलाइज़्ड टेबल व्यूपोर्ट से बाहर स्क्रॉल होने वाले आइटम्स के लिए कैश डिस्कार्ड कर सकता है। यदि आप `useCallback` को परफॉर्मेंस ऑप्टिमाइज़ेशन के लिए उपयोग कर रहे हैं, तो यह आपके अपेक्षाओं के अनुरूप होना चाहिए। अन्यथा, [स्टेट वेरिएबल](/reference/react/useState#im-trying-to-set-state-to-a-function-but-it-gets-called-instead) या [रेफ़](/reference/react/useRef#avoiding-recreating-the-ref-contents) बेहतर विकल्प हो सकता है।

---

## Usage {/*usage*/}

### कम्पोनेंट की री-रेंडरिंग को स्किप करना {/*skipping-re-rendering-of-components*/}

जब आप रेंडर परफॉर्मेंस को ऑप्टिमाइज़ करते हैं, तो चाइल्ड कम्पोनेंट को पास किए गए फंक्शन्स को कैश करना कभी-कभी ज़रूरी होता है। पहले इसका सिंटैक्स देखें, फिर समझें कि यह कब उपयोगी है।

अपने कम्पोनेंट के रेंडर के बीच फंक्शन को कैश करने के लिए, उसकी डेफिनिशन को `useCallback` हुक में रैप करें:
...
2. <CodeStep step={2}>डिपेंडेंसीज़ की सूची</CodeStep>, जिसमें आपके कम्पोनेंट के वे सभी वैल्यूज़ शामिल हों जो इस फंक्शन में उपयोग हुए हैं।
...
मान लें आप `ProductPage` कम्पोनेंट से `handleSubmit` फंक्शन को `ShippingForm` कम्पोनेंट में पास कर रहे हैं:
...
**डिफॉल्ट रूप से, जब कोई कम्पोनेंट री-रेंडर होता है, तो React उसके सभी चाइल्ड कम्पोनेंट्स को रिकर्सिवली री-रेंडर करता है।** इसलिए जब `ProductPage` अलग `theme` के साथ री-रेंडर होता है, तो `ShippingForm` भी री-रेंडर होता है। यदि चाइल्ड कम्पोनेंट हल्का है, तो यह सामान्य है। लेकिन यदि आपने चेक किया और पाया कि री-रेंडर धीमा है, तो आप `ShippingForm` को [`memo`](/reference/react/memo) से रैप करके बता सकते हैं कि यदि उसके प्रॉप्स पिछले रेंडर जैसे ही हैं, तो री-रेंडर स्किप करें:
...
`useCallback` अक्सर [`useMemo`](/reference/react/useMemo) के साथ उपयोग होता है। दोनों चाइल्ड कम्पोनेंट की परफॉर्मेंस ऑप्टिमाइज़ करने में उपयोगी हैं। ये आपको नीचे पास की गई वैल्यूज़ को [मेमोइज़](https://en.wikipedia.org/wiki/Memoization) (कैश) करने की सुविधा देते हैं:
...
* जब आप इसे किसी [`memo`](/reference/react/memo) में रैप किए गए कम्पोनेंट को प्रॉप के रूप में पास करते हैं। आप चाहते हैं कि वैल्यू न बदलने पर री-रेंडरिंग स्किप हो जाए। मेमोइज़ेशन से कम्पोनेंट तभी री-रेंडर होगा जब उसकी डिपेंडेंसीज़ बदलेंगी।
...
अन्य मामलों में `useCallback` से फंक्शन को रैप करने का विशेष फ़ायदा नहीं है। हालाँकि ऐसा करने से बड़ा नुकसान भी नहीं होता, लेकिन कोड कम रीडेबल हो सकता है। साथ ही, हर मेमोइज़ेशन प्रभावी नहीं होता: एक भी "हर बार नई" वैल्यू पूरे कम्पोनेंट के मेमोइज़ेशन को बेकार कर सकती है।
...
1. जब कोई कम्पोनेंट अन्य कम्पोनेंट्स को विज़ुअली रैप करता है, तो उसे [JSX को चिल्ड्रन के रूप में स्वीकार](/learn/passing-props-to-a-component#passing-jsx-as-children) करने दें। इससे यदि रैपर कम्पोनेंट अपने स्टेट को अपडेट करता है, तो React जानता है कि उसके चिल्ड्रन को री-रेंडर करने की ज़रूरत नहीं है।

2. लोकल स्टेट का उपयोग करें और ज़रूरत से ज़्यादा [स्टेट को ऊपर न ले जाएँ](/learn/sharing-state-between-components)। फ़ॉर्म जैसे ट्रांज़िएंट स्टेट या आइटम होवर की जानकारी जैसे स्टेट्स को कम्पोनेंट ट्री के सबसे ऊपर या ग्लोबल स्टेट लाइब्रेरी में न रखें।

3. अपने [रेंडरिंग लॉजिक को प्योर रखें](/learn/keeping-components-pure)। यदि कम्पोनेंट के री-रेंडर होने पर समस्या होती है या विज़ुअल ग्लिच दिखता है, तो यह कम्पोनेंट का बग है। मेमोइज़ेशन जोड़ने के बजाय बग को ठीक करें।

4. [अनावश्यक स्टेट अपडेट्स करने वाले इफ़ेक्ट्स](/learn/you-might-not-need-an-effect) से बचें। React ऐप्स की अधिकतर परफॉर्मेंस समस्याएँ ऐसे इफ़ेक्ट्स से होती हैं जो अपडेट की चेन शुरू करते हैं, जिससे कम्पोनेंट बार-बार रेंडर होते हैं।

5. इफ़ेक्ट्स में से [अनावश्यक डिपेंडेंसीज़ हटाएँ](/learn/removing-effect-dependencies)। उदाहरण के लिए, मेमोइज़ेशन की बजाय ऑब्जेक्ट या फंक्शन को इफ़ेक्ट के अंदर या कम्पोनेंट के बाहर रखना आसान होता है।
...
इस उदाहरण में, `ShippingForm` कम्पोनेंट को **जानबूझकर धीमा किया गया है** ताकि आप देख सकें कि जब कोई React कम्पोनेंट वास्तव में धीमा हो, तब क्या होता है। काउंटर बढ़ाने और थीम को टॉगल करने की कोशिश करें।
...
#### कम्पोनेंट को हमेशा री-रेंडर करना {/*always-re-rendering-a-component*/}

इस उदाहरण में, `ShippingForm` की इम्प्लिमेंटेशन को **जानबूझकर धीमा किया गया है**, ताकि आप देख सकें कि जब कोई React कम्पोनेंट वास्तव में धीमा हो, तब क्या होता है। काउंटर बढ़ाने और थीम को टॉगल करने की कोशिश करें।

पिछले उदाहरण के विपरीत, अब थीम को टॉगल करना भी धीमा है! इसका कारण यह है कि **इस वर्ज़न में `useCallback` का उपयोग नहीं किया गया है,** इसलिए `handleSubmit` हर बार नया फंक्शन बनता है, और धीमा किया गया `ShippingForm` कम्पोनेंट री-रेंडरिंग स्किप नहीं कर पाता।
...
### हर बार मेरे कम्पोनेंट के रेंडर होने पर `useCallback` नया फंक्शन रिटर्न करता है {/*every-time-my-component-renders-usecallback-returns-a-different-function*/}
...
मान लें `Chart` कम्पोनेंट को [`memo`](/reference/react/memo) में रैप किया गया है। आप चाहते हैं कि `ReportList` कम्पोनेंट के री-रेंडर होने पर लिस्ट में हर `Chart` का री-रेंडर स्किप हो जाए। लेकिन आप लूप के अंदर `useCallback` कॉल नहीं कर सकते:
...
इसके बजाय, विशिष्ट आइटम के लिए अलग कम्पोनेंट बनाएँ और उसमें `useCallback` रखें:
...
या फिर, आप `useCallback` हटा सकते हैं और `Report` कम्पोनेंट को [`memo`](/reference/react/memo) में रैप कर सकते हैं। यदि `item` प्रॉप नहीं बदलता, तो `Report` री-रेंडर स्किप करेगा, जिससे `Chart` भी री-रेंडर स्किप कर देगा:
