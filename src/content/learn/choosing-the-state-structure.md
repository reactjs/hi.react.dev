...
State को अच्छी तरह से स्ट्रक्टरिंग करने से, एक कौम्पोनॅन्ट में मॉडिफाई और डीबग करने में आनंददायक अंतर बन सकता है, जबकि एक ऐसा कौम्पोनॅन्ट एक निरंतर बग के स्रोत के रूप में दिखाई देगा। यहां state को स्ट्रक्टरिंग करते समय आपको ध्यान में रखने चाहिए कुछ टिप्स हैं।
...
* State का ऑर्गनाइज़ करते समय किन बातों से बचना चाहिए
...
जब आप एक कौम्पोनॅन्ट लिखते हैं जो कुछ state रखता है, तो आपको कितने state वेरिएबल्स का उपयोग करना है और उनके डेटा का शेप क्या होना चाहिए, इसके बारे में आपको चुनाव करना होगा। हालांकि एक सबऑप्टिमल state स्ट्रक्चर के साथ भी सही प्रोग्राम लिखना पॉसिबल है, कुछ प्रिंसिपल्स हैं जो आपको बेहतर विकल्प बनाने के लिए मार्गदर्शन कर सकते हैं:

1. **ग्रुप से संबंधित state।** यदि आप हमेशा एक ही समय में दो या दो से ज्यादा state वेरिएबल्स को अपडेट करते हैं, तो उन्हें एक state वेरिएबल में मर्ज करें।
...
3. **रेडंडेंट state से बचें।** यदि आप रेंडरिंग के दौरान कौम्पोनॅन्ट के props या उसके मौजूदा state वेरिएबल्स से कुछ जानकारी कैलकुलेट कर सकते हैं, तो आपको उस जानकारी को उस कौम्पोनॅन्ट के state में नहीं रखना चाहिए।
...
तकनीकी तौर पर, आप इनमें से किसी भी तरीके का इस्तेमाल कर सकते हैं। लेकिन **अगर कुछ दो state वेरिएबल्स हमेशा एक साथ बदलते हैं, तो यह एक अच्छा विचार हो सकता है कि उन्हें एक state वेरिएबल में एकीकृत किया**। तब आप उन्हें हमेशा सिंक में रखना नहीं भूलते हैं, जैसे कि इस उदाहरण में, जहां कर्सर को आगे बढ़ाना लाल डॉट के दोनों कोआर्डिनेट्स को अपडेट करता है:
...
एक और मामला जहां आप डेटा को किसी ऑब्जेक्ट या array में समूहित करेंगे, जब आप नहीं जानते कि आपको state के कितने अलग-अलग टुकड़ों की ज़रूरत होगी। उदाहरण के लिए, यह तब मददगार होता है जब आपके पास एक फॉर्म होता है जहां उपयोगकर्ता कस्टम फ़ील्ड जोड़ सकता है।
...
हालांकि यह कोड काम करता है, लेकिन यह "नामुमकिन" states के लिए दरवाजा खुला छोड़ देता है। उदाहरण के लिए, यदि आप एक साथ `setIsSent` और `setIsSending` को कॉल करना भूल जाते हैं, तो आप ऐसी स्थिति में हो सकते हैं जहाँ `isSending` और` isSent` दोनों एक ही समय में `true` हैं। आपका कौम्पोनॅन्ट जितना जटिल होगा, यह समझना उतना ही मुश्किल होगा कि क्या हुआ था।

**चूँकि `isSending` और` isSent` एक ही समय में कभी भी `true` नहीं होने चाहिए, इसलिए बेहतर होगा कि उन्हें एक `status` state वेरिएबल से बदल दिया जाए जो _तीन_ मान्य states में से एक ले सकता है:** `'typing'` (शुरुआती), `'sending'`, और `'sent'`::
...
पढ़ने की क्षमता के लिए आप अभी भी कुछ कॉन्स्टेंट्स घोषित कर सकते हैं:
...
## रेडंडेंट state से बचें {/*avoid-redundant-state*/}

यदि आप रेंडरिंग के दौरान कौम्पोनॅन्ट के props या उसके मौजूदा state वेरिएबल्स से कुछ जानकारी की गणना कर सकते हैं, तो आपको उस जानकारी को उस कौम्पोनॅन्ट के state में **नहीं डालना** चाहिए।

उदाहरण के लिए, इस फॉर्म को लें। यह काम करता है, लेकिन क्या आप इसमें कोई रेडंडेंट state ढूंढ सकते हैं?
...
इस फॉर्म में तीन state वेरिएबल्स हैं: `firstName`, `lastName`, and `fullName`. हालांकि, `fullName` रेडंडेंट है। **आप रेंडर के दौरान हमेशा `firstName` और`lastName` से `fullName` की गणना कर सकते हैं, इसलिए इसे state से हटा दें।**
...
यहां, `fullName` एक state वेरिएबल _नहीं_ है। इसके बजाय, इसकी गणना रेंडर के दौरान की जाती है:

```js
const fullName = firstName + ' ' + lastName;
```

nge handler को इसे अपडेट करने के लिए कुछ विशेष करने की ज़रूरत नहीं है। जब आप `setFirstName` या `setLastName` को कॉल करते हैं, तो आप फिर से रेंडर ट्रिगर करते हैं, और फिर अगले `fullName` की गणना नए डेटा से की जाएगी।

<DeepDive>

#### Don't mirror props in state {/*don-t-mirror-props-in-state*/}

रेडंडेंट state का एक सामान्य उदाहरण इस तरह का कोड है:
...
यहाँ, एक `color` state वेरिएबल को `messageColor` prop में इनिशियलाइज़ किया गया है। समस्या यह है कि **यदि पैरेंट कौम्पोनॅन्ट बाद में `messageColor` का एक अलग वैल्यू पास करता है (उदाहरण के लिए, `'blue'` के बजाय `'red'`), तो `color` _state वेरिएबल_ अपडेट नहीं किया जाएगा!** State केवल पहले रेंडर के दौरान इनिशलाइज्ड होता है।

यही कारण है कि state वेरिएबल में कुछ prop को "मिररिंग" करने से कन्फ्यूजन पैदा हो सकता है। इसके बजाय, सीधे अपने कोड में `messageColor` props का उपयोग करें। यदि आप इसे छोटा नाम देना चाहते हैं, तो कॉन्स्टेंट का उपयोग करें:
...
State में "मिररिंग" prop तभी समझ में आता है जब आप किसी विशिष्ट prop के लिए सभी अपडेट को अनदेखा करना चाहते हैं। प्रथा के अनुसार, prop नाम को `initial` या `default` से शुरू करें ताकि यह स्पष्ट किया जा सके कि इसके नए वैल्यूज़ को अनदेखा किया गया है:ed:
...
इटम पर "Choose" पर क्लिक करते हैं और फिर उसे _एडिट_ करते हैं, **इनपुट अपडेट होता है लेकिन नीचे का लेबल एडिट को रिफ्लेक्ट नहीं करता है।** ऐसा इसलिए है क्योंकि आपके पास डुप्लिकेट state है, और आप `selectedItem` को अपडेट करना भूल गए हैं।

पर आप `selectedItem` को इसमें अपडेट कर सकते हैं, डुप्लिकेट को निकालना आसान हल है. इस उदाहरण में, एक `selectedItem` ऑब्जेक्ट के बजाय (जो `items` के अंदर ऑब्जेक्ट्स के साथ डुप्लिकेशंस बनाता है), आप state में `selectedId` रखते हैं, और _फिर_ किसी item के लिए `items` array खोजकर `selectedItem` प्राप्त करते हैं उस ID के साथ:
...
अब अगर आप _selected_ item को एडिट करते हैं, तो नीचे दिया गया मैसेज तुरंत अपडेट हो जाएगा। ऐसा इसलिए है क्योंकि `setItems` फिर से रेंडर ट्रिगर करता है, और `items.find(...)` आइटम को अपडेट किया गया title के साथ ढूंढेगा। आपको _selected item_ को state में रखने की ज़रूरत नहीं थी, क्योंकि केवल _selected ID_ ही ज़रूरी है। बाकी की गणना रेंडर के दौरान की जा सकती है।
...
**यदि state आसानी से अपडेट करने के लिए नेस्टेड है, तो इसे "फ्लैट" बनाने पर विचार करें।** यहाँ एक तरीका है जिससे आप इस डेटा को रीस्ट्रक्चर कर सकते हैं। एक पेड़ जैसी स्ट्रक्चर के बजाय जहां प्रत्येक `place` में _child places_ की एक array होती है, आपके पास प्रत्येक स्थान में _child place IDs_ की एक array हो सकती है। फिर आप प्रत्येक place ID से संबंधित स्थान पर मैपिंग स्टोर कर सकते हैं।
...
* रेडंडेंट और डुप्लिकेट स्थिति से बचें ताकि आपको इसे सिंक में रखने की ज़रूरत न हो।
* जब तक आप विशेष रूप से अपडेट को रोकना नहीं चाहते हैं, तब तक props को state _में_ न रखें।
* UI पैटर्न जैसे सिलेक्शन के लिए, ऑब्जेक्ट के बजाय ID या index को state में रखें।
* यदि गहराई से नेस्टेड state को अपडेट करना जटिल है, तो इसे फ़्लैट करने का प्रयास करें।
...
समस्या को ठीक करने के लिए, state से डुप्लिकेशन हटाएं। _letter_ को दो जगह स्टोर करने की बजाय, `highlightedId` स्टोर करें। फिर आप प्रत्येक अक्षर के लिए `letter.id === HighlightedId` के साथ `isHighlighted` की जांच कर सकते हैं, जो अंतिम रेंडर के बाद से `letter` ऑब्जेक्ट बदल जाने पर भी काम करेगा।
...
एक `selectedId` के बजाय, `selectedIds` _array_ को state में रखें। उदाहरण के लिए, यदि आप पहले और अंतिम अक्षर का सिलेक्शन करते हैं, इसमें `[0, 2]` होगा। जब कुछ भी नहीं चुना जाता है, तो यह एक खाली `[]` array होगी:
...
इसे ठीक करने के लिए, आप [Set](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Set) को state में रख सकते हैं, जो एक तेज़ [`has()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Set/has) ऑपरेशन है:
...
ध्यान रखें कि आप [state में ऑब्जेक्ट्स को म्यूटेट नहीं करना चाहिए](/learn/updating-objects-in-state), और इसमें Sets भी शामिल हैं। यही कारण है कि `handleToggle` फ़ंक्शन पहले सेट की एक _कॉपी_ बनाता है, और फिर उस कॉपी को अपडेट करता है।
